...
export default function TutorPage() {
  ...
  // Sync SpeechSynthesizer mute state with voice toggle
  useEffect(() => {
    speechSynthesizer.setMuted(!voiceEnabled);
  }, [voiceEnabled]);

  // Voice Speech Engine powered by SpeechSynthesizer & Phonetic Viseme Parser
  ...
  useEffect(() => {
    ...
    const speak = async (
      ...
      const speechPromise = speechSynthesizer.speak(text, {
        gender: isMale ? 'male' : 'female',
        rate: 1.0,
        pitch: isMale ? 0.95 : 1.05
      });
        ...
        <div className="w-[36%] h-full relative border-r border-hexagon-border bg-gradient-to-b from-[#0e1217] to-[#080a0d] overflow-hidden flex flex-col">
          ...
          <div className="absolute top-4 left-4 right-4 flex items-center justify-between z-20 pointer-events-auto">
            ...
            <div className="flex items-center gap-2">
              <button 
                onClick={() => {
                  const testPhrase = "Listen closely and observe my lip articulation! Every vowel and consonant shapes my mouth with exact phonemes, from open vowels to closed lips.";
                  setCaption(testPhrase);
                  setTeacherState('teaching', testPhrase);
                  speechSynthesizer.speak(testPhrase, {
                    gender: isMale ? 'male' : 'female',
                    rate: 1.0,
                    pitch: isMale ? 0.95 : 1.05
                  });
